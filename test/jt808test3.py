...
class JT808Parser(object):
    ...
    def SaveCSV(self, msgList, csvFile):
        sequence = ("DevNum", "MsgId", "SeqNo")
        ...
        def RowData(msg):
            ...
            if None != ackId:
                rdata.append("0x%04X" % ackId)
                rdata.append(msg.get("AckSeq", "-"))
                #temp = "结果:%d (%s)" % (msg.get("Result"), msg.get("ResultText"))
                temp = "%d (%s)" % (msg.get("Result"), msg.get("ResultText"))
                rdata.append(temp)
            elif 0x0200 == msgId and msg.get("Time") != None:
                rdata.append("-")
                rdata.append("-")
                rdata.append("-")
                rdata.append(msg["Time"])
                temp = "%f,%f" % (msg["Latitude"], msg["Longitude"])

                if msg.get("Altitude") != None:
                    temp = temp + ",高度=%d" % msg.get("Altitude")

                if msg.get("Speed") != None:
                    temp = temp + ",速度=%.02f" % msg.get("Speed")

                if msg.get("Angle") != None:
                    temp = temp + ",角度=%d" % msg.get("Angle")

                rdata.append(temp)
                temp = ""
                for text in msg["Alarm"][1]:
                    #temp = temp + text.strip("报警") + ","
                    temp = temp + text.strip() + ","
                if len(temp) == 0:
                    temp = "无报警"
                temp = "报警:%s (%s)" % (msg["Alarm"][0], temp.strip(","))
                rdata.append(temp)

                temp = ""
                for text in msg["Status"][1]:
                    temp = temp + text + ','
                temp = "状态:%s (%s)" % (msg["Status"][0], temp.strip(","))
                rdata.append(temp)
        ...
        if len(msgList) == 0:
            return
        try:
            with open(csvFile, 'w', newline='') as fileObj:
                csv_write = csv.writer(fileObj, dialect='excel')
                title = ("设备号", "命令ID", "流水号", "应答ID", "应答流水号", "结果")
                csv_write.writerow(title)
                for msg in msgList:
                    csv_write.writerow(RowData(msg))
                fileObj.close()
        except Exception as e:
            self.LOGE(str(e))

    def SaveCSV0200(self, msgList, csvFile):
        def Msg0200Data(msgList):
            rowList = []
            rowTitle = ["Time", ]
            for msg in msgList:
                if msg.get("MsgId") != 0x0200:
                    continue
                rowBuffer = []

                for k in msg.keys():
                    if k in ('MsgId', 'Attr', 'BodyData', 'Length', 'DevNum'):
                        pass
                    elif k in rowTitle:
                        pass
                    else:
                        rowTitle.append(k)
                #print(len(msg), len(rowTitle))
                for t in rowTitle:
                    if msg.get(t) == None:
                        rowBuffer.append("")
                    else:
                        ss = str(msg.get(t)).strip()
                        if t in ('Alarm', 'Status'):
                ...
                rowList.append(tuple(rowBuffer))
                del(rowBuffer)
            ...
            for t in rowTitle:
                if t == 'Speed':
                    ...
                    tmpTitle.append('里程(KM)')
                elif t == 'Voltage':
                    tmpTitle.append('电压(伏)')
                elif t == 'Percent':
                    tmpTitle.append('电量')
                elif t == 'Vib10S':
                    tmpTitle.append('10秒震动')
                elif t == 'Vib30S':
                    tmpTitle.append('30秒震动')
                elif t == 'Angle':
                    tmpTitle.append('方向')
                elif t == 'Sats':
                    tmpTitle.append('卫星数')
                    ...
                    tmpTitle.append(t)
            rowList.insert(0, tuple(tmpTitle))
            return rowList
        ...
        try:
            with open(csvFile, 'w', newline='') as fileObj:
                csv_write = csv.writer(fileObj, dialect='excel')
                for row in Msg0200Data(msgList):
                    csv_write.writerow(row)
                fileObj.close()
        except Exception as e:
            self.LOGE(str(e))

    def SaveKml(self, msgList, kmlFile, kmlName="JT808"):
        locationList = []
        for m in msgList:
            if m.get("MsgId") != 0x0200:
                continue
            loc = self.JT808ToLocation(m)
            if None != loc:
                locationList.append(loc)

        try:
            if len(locationList) > 0:
                KM.CreateKmlFile(locationList, kmlName, kmlFile, True)
        except Exception as e:
            self.LOGE("CreateKmlFile:" + str(e))
    ...
    def PaserAndSave(self, logs, name, path=''):
        ...
        print(len(result))
        csvPath = os.path.join(path, name + ".csv")
        csv0200Path = os.path.join(path, name + "_0200.csv")
        kmlPath = os.path.join(path, name + ".kml")

        self.SaveCSV(result, csvPath)
        self.SaveCSV0200(result, csv0200Path)
        # name = "JT808_" + name
        self.SaveKml(result, kmlPath, name)
